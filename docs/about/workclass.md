...
### text-align属性
  ...
  2. text-align属性只对块级元素起作用 内联级元素无效
...
### CSS3新增UI元素伪类
> :disable(input框的未选中的伪类)  :enabled(input框的可选中的伪类)  :checked(单选框或者复选框的选中伪类)
...
- scale() 缩放 参数为缩放比例  缩放时水平方向竖直方向的坐标原点在元素中心
> scaleX()
> scaleY()
> scale(X,Y)
- skew() 扭曲与斜切
...
### CSS3 3D转换
- rotate3d()
> rotateX() 对象在x轴上旋转 90deg时图片消失 
> rotateY() 在y轴旋转 向里旋转为正角度
